...
steps = GUIRestingState.EmptyStep(0);
...
steps(end+1) = step("1.1", "Import", "Import", true, ...
    "1-1_Import", [
    param("WriteRawBIDS", "Write raw BIDS files", "logical", "none", false, {}, ...
    "If true, writes a copy of the organized raw data in BIDS format (SNIRF and sidecar files). This can be very slow on some systems.", true, false)
...
steps(end+1) = step("1.2", "Verify Montage", "Import", true, ...
    "1-2_Verify", GUIRestingState.EmptyParam(0), "VerifyMontages");
...
steps(end+1) = step("3.1", "Optical Density", "Preprocessing", true, ...
    "3-1_Optical-Density", GUIRestingState.EmptyParam(0), "OpticalDensity");

steps(end+1) = step("3.2", "Temporal Derivative Distribution Repair", "Preprocessing", false, ...
    "3-2_TDDR", GUIRestingState.EmptyParam(0), "TDDR");
...
steps(end+1) = step("3.11", "Group Connectivity: Figures", "Preprocessing", false, ...
    ...
    function s = step(id, name, phase, mandatory, figFolder, parameters, fcn)
        ...
        if isempty(parameters)
            s.Parameters = GUIRestingState.EmptyParam(0);
        else
    ...
    function p = param(name, label, type, validation, value, options, description, marksDirty, propagatesDirty)
        ...
        p = GUIRestingState.EmptyParam(1);
        p.Name = name;
        p.Label = label;
        p.Type = type;
