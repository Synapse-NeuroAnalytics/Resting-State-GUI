...
if ~app.IsRunning
    GUIRestingState.Utils.Log("Stop requested, but no run is currently in progress.", app);
    return
end

app.StopRequested = true;
app.StopButton.Enable = "off";  % avoid duplicate requests while it winds down
GUIRestingState.Utils.Log("Stopping run after the current step finishes...", app);
end
