...
switch choice
    case "New pipeline"
        pipeline = GUI.Windows.NewPipelineDialog(app);
    case "Load pipeline"
        [f, p] = uigetfile("*.mat", "Load pipeline");
        if ~isequal(f, 0)
            filepath = fullfile(p, f);
            pipeline = GUI.Utils.LoadPipelineFile(app, string(filepath));
        end
    otherwise
        % Cancel option
end
...
GUI.UI.BuildStepRows(app);
GUI.Utils.Log("fNIRS Preprocessing GUI started.", app);
end
