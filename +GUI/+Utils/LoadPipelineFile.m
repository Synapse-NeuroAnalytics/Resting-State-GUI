...
%
% Returns the loaded (and possibly path-corrected) GUI.Pipeline object,
...
try
    candidate = GUI.Pipeline.Load(filepath);
catch ME
...
if strlength(oldPipelinePath) > 0 && oldPipelinePath ~= newPipelinePath
    [oldRoot, newRoot] = GUI.Utils.FindCommonRootSwap(oldPipelinePath, newPipelinePath);
    if strlength(oldRoot) > 0
        candidate = GUI.Utils.ApplyRootSwap(candidate, oldRoot, newRoot);
        GUI.Utils.Log(sprintf("Pipeline opened from a different location than saved ('%s' -> '%s'); auto-matched paths where possible.", ...
            oldRoot, newRoot), app);
    end
end

required = GUI.Pipeline.RequiredPaths(candidate);
...
if ~isempty(missing)
    ...
    if choice == "Abort"
        GUI.Utils.Log("Load pipeline aborted (missing paths).", app);
        loaded = [];
        return
    end
    ...
    updatedPaths = GUI.Windows.NewPipelineDialog(app, "redefine", existingValues);
    if isempty(updatedPaths)
        GUI.Utils.Log("Load pipeline aborted (paths not redefined).", app);
        loaded = [];
        return
    end
