...
enabledCb.ValueChangedFcn = @(src, event) GUI.Callbacks.StepEnabledChanged(app, event, i);
...
if isempty(s.Parameters)
    ...
    for p = 1:numParams
        ...
        ctrl.ValueChangedFcn = @(src, event) GUI.Callbacks.ParameterValueChanged(app, event, i, p);
...
genFigCb.ValueChangedFcn = @(src, event) GUI.Callbacks.GenerateFiguresChanged(app, event, i);

openFigBtn = uibutton(figGrid, "Text", "open figure folder", "FontSize", 10, ...
    "BackgroundColor", [0.498, 0.498, 0.498], "FontColor", "white", ...
    "ButtonPushedFcn", @(src, event) GUI.Callbacks.OpenStepFigureFolderPushed(app, event, i));
...
runOnlyBtn = uibutton(runGrid, "Text", sprintf("run\nonly"), "FontSize", 10, ...
    ...
    "ButtonPushedFcn", @(src, event) GUI.Callbacks.RunOnlyPushed(app, event, i));

runToBtn = uibutton(runGrid, "Text", sprintf("run to\nhere"), "FontSize", 10, ...
    "BackgroundColor", [0.706, 0.722, 0.604], "FontColor", [0.32 0.15 0.07], ...
    "ButtonPushedFcn", @(src, event) GUI.Callbacks.RunToHerePushed(app, event, i));

runFromBtn = uibutton(runGrid, "Text", sprintf("run from\nhere"), "FontSize", 10, ...
    "BackgroundColor", [0.706, 0.722, 0.604], "FontColor", [0.32 0.15 0.07], ...
    "ButtonPushedFcn", @(src, event) GUI.Callbacks.RunFromHerePushed(app, event, i));
