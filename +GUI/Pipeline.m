    ...
    methods
        ...
        function RunSteps(obj, indices, app)
            ...
            if hasStopSupport
                ...
                app.IsRunning = true;
                GUI.Utils.SetRunControlsEnabled(app, false);
            end
            ...
            % Initialize fNIRS-Preprocessing Pipeline (unless GUI.Pipeline.TEST_MODE)
            if ~GUI.Pipeline.TEST_MODE
                pipeline = Pipeline;
            ...
            for k = 1:numel(indices)
                if hasStopSupport && app.StopRequested
                    GUI.Utils.Log(sprintf("Run stopped by user before step %s.", ...
                ...
                if ~s.Enabled
                    GUI.Utils.Log(sprintf("Skipping step %s (disabled)", s.Name), app);
                    drawnow;
                    continue
                end

                GUI.Utils.Log(sprintf("Running step %s (%s)...", s.ID, s.Name), app);
                ...
                GUI.Utils.Log(sprintf("Finished step %s", s.ID), app);

                GUI.Utils.MarkDownstreamDirty(app);
                drawnow;  % repaint badge color / log immediately, don't wait for the whole run to finish
            end

            if ~wasStopped
                GUI.Utils.Log("All selected steps have completed.", app);
            end
        end
        ...
        function Save(obj, filepath)
            ...
            GUI.Utils.Log(sprintf("Pipeline saved to %s", filepath));
        end
    end
...
app.IsRunning = false;
GUI.Utils.SetRunControlsEnabled(app, true);
end